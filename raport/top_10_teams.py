...
max_rounds = cs_merged.groupby('year')['round'] \
    .max().reset_index() \
...
champ_counts = champions['constructorId'] \
    .value_counts() \
...
top10 = top10.merge(cons[['constructorId', 'name']], on='constructorId') \
...
plt.barh(top10_plt['name'], top10_plt['championships'], color='orange')
plt.xlabel('Number of Championships')
plt.ylabel('Constructor')
plt.title("Top 10 Constructors by Number of Constructors' Championships")
plt.tight_layout()
plt.show()

# 11. Interaktywny wykres w Plotly
fig = px.bar(
    top10,
    x='name',
    y='championships',
    title="Top 10 Constructors by Number of Constructors' Championships",
    labels={'name': 'Constructor', 'championships': 'Number of Championships'}
)
